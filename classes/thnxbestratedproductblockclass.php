...
class thnxbestratedproductblockclass extends ObjectModel
{
	public $id;
	public $id_thnxbestratedproductblock;
	public $position;
	public $hook;
	public $content;
	public $title;
	...
	public $pages;
	private static $module_name = 'thnxbestratedproductblock';
	private static $tablename = 'thnxbestratedproductblock';
	private static $classname = 'thnxbestratedproductblockclass';
	public static $definition = array(
		'table' => 'thnxbestratedproductblock',
		'primary' => 'id_thnxbestratedproductblock',
		'multilang' => true,
		'fields' => array(
    ...
    public static function GetProductBlock($hook = NULL)
    {
        ...
        $product_type = 'bestrated_product';
        $product_item = NULL;
        ...
        if(isset($allproducts) && !empty($allproducts)){
            ...
            foreach ($allproducts as $allprds){
                ...
                if($module_name::PageException($allprds['pages'])){
                    ...
                    if(Module::isInstalled('productcomments') || Module::isInstalled('thnxproductcomments')){
                    	$result_getProducts = self::GetProductProperties($product_type,$product_item,$count_prd,$order_by,$order_way);
                    }else{
                    	$result_getProducts = self::GetProductProperties("new_product",$product_item,$count_prd,$order_by,$order_way);
                    }
                    if(empty($result_getProducts)){
                    	$result_getProducts = self::GetProductProperties("new_product",$product_item,$count_prd,$order_by,$order_way);
                    }
                    // start 1.7
    				// $result_getProducts = self::GetProductProperties($product_type,$product_item,$count_prd,$order_by,$order_way);
    ...
    public static function GetOnSaleProducts($limit = 5)
    {
            $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity,
             pl.`description`, pl.`description_short`, product_attribute_shop.id_product_attribute, pl.`link_rewrite`,
              pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, image_shop.`id_image`, il.`legend`,
            ...
            ' LEFT JOIN `' . _DB_PREFIX_ . 'image_lang` il ON (i.`id_image` = il.`id_image` AND il.`id_lang` = ' . (int) Context::getContext()->language->id . ')
            LEFT JOIN `' . _DB_PREFIX_ . 'manufacturer` m ON (m.`id_manufacturer` = p.`id_manufacturer`) WHERE  p.`on_sale` = 1  AND product_shop.`active` = 1  
            AND product_shop.`show_price` = 1 AND ((image_shop.id_image IS NOT NULL OR i.id_image IS NULL) OR (image_shop.id_image IS NULL AND i.cover=1)) AND (pa.id_product_attribute IS NULL OR 
                product_attribute_shop.default_on = 1) LIMIT '.$limit;
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
    ...
    public static function GetProductProperties($product_type='new_product',$product_item=NULL,$limit=10,$order_by='id_product',$order_way='DESC')
    {
        ...
        $context = Context::getcontext();
            ...
            }elseif($product_type == 'bestrated_product'){
                    $productproperties = self::GetBestRatedProductID($limit);
            }elseif($product_type == 'onsale_product'){
                    $productproperties = self::GetOnSaleProducts($limit);
            }
        return $productproperties;
    }
    public static function GetBestRatedProductID($limit = 5)
	{
		$results = array();
		$sql = "SELECT (SUM(pc.`grade`) / COUNT(pc.`grade`)) AS avg,pc.`id_product` AS product FROM `". _DB_PREFIX_ ."thnxproduct_comment` pc WHERE pc.`deleted` = 0 group by pc.`id_product` ORDER BY avg DESC LIMIT {$limit}";
		$products = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
		if(isset($products) && !empty($products)){
			$i = 0;
			foreach($products as $product){
				$results[$i] = self::GetAllProductProperties($product['product']);
				$i++;
			}
		}
		return Product::getProductsProperties((int) Context::getContext()->language->id,$results);
	}
	public static function GetAllProductProperties($id = NULL)
	{
	    if($id == NULL)
	        return false;
